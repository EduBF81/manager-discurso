/**
 * Módulo de Temas S-99
 * @description Centraliza a lista real oficial de temas de discursos públicos.
 * Esta lista é exportada ditada do legacy_backup como um array de objetos fortemente tipado.
 */

export interface TemaS99 {
    numero: number;
    titulo: string;
}
...
// Exporta o array tipado a partir do parser
export const temasFormatados: TemaS99[] = textoBrutoS99
    .split('\n')
    ...
    .map(linha => {
        ...
        if (partes.length >= 2) {
            return {
                numero: parseInt(partes[0].trim(), 10),
                titulo: partes.slice(1).join('.').trim()
            };
        }
        return null;
    })
    .filter((item): item is TemaS99 => item !== null);
